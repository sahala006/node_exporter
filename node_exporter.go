...
import (
	"fmt"
	"github.com/prometheus/node_exporter/reporter"
	"log/slog"
	"net/http"
	_ "net/http/pprof"
	"os"
...
func main() {
	var (
		cmdbAddress = kingpin.Flag(
			"cmdb.address",
			"cmdb Address, asset will be reported to this url ",
		).Default("https://xxx.com:443").String()
	...
	go reporter.Report(logger, *cmdbAddress)
	server := &http.Server{}
